...
bool FasterrLocalSimulator::Start(std::ostream& out) {
  ...
  Stop * stop_CC_WB_3 = new Stop(7, 44.983703, -93.178846);  // Fairgrounds
  Stop * stop_CC_WB_4 = new Stop(8, 44.980663, -93.180808);  // Tway
  Stop * stop_CC_WB_5 = new Stop(9, 44.976397, -93.221801);  // before transit
  ...
  std::list<double> CC_EB_probs;  // realistic .15, .3, .025, .05, 0
  CC_EB_probs.push_back(.15);  // before-transit
  CC_EB_probs.push_back(.3);  // Tway
  CC_EB_probs.push_back(.025);  // Fairgrounds
  CC_EB_probs.push_back(.05);  // Buford & Gartner
  CC_EB_probs.push_back(0);  // SPSC - MUST BE 0
  // is this always true? If so, we may want to reduce the length of
  // probs to_char_type
  //        remove possibility of generating passengers with nowhere to go

  std::list<double> CC_WB_probs;  // realistic .35, .05, .01, .01, 0
  CC_WB_probs.push_back(.35);   // SPSC
  CC_WB_probs.push_back(.05);  // Buford & Gartner
  CC_WB_probs.push_back(.01);  // Fairgrounds
  CC_WB_probs.push_back(.01);  // Tway
  CC_WB_probs.push_back(0);  // before-transit
...
bool FasterrLocalSimulator::Update(std::ostream& out) {
  // increase time
